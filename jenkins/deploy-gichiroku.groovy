pipeline {
    ...
    environment {
        ...
        PROJECT_ID = "ai-agent-hackathon-temporary"
        GCP_REGION = "asia-northeast1"
        IMAGE_NAME = "cloud-run-image"
        PRIMARY_IMAGE_FULL_PATH = "${env.GCP_REGION}-docker.pkg.dev/${env.PROJECT_ID}/${env.REPO_NAME}/${env.REPO_BRANCH}:latest"
    }
    
    parameters {
            string(name: 'REPO_BRANCH', defaultValue: 'create-cloud-run-jenkinsfile', description: 'デプロイするGitリポジトリのブランチ')
            booleanParam(name: 'IS_BUILD', defaultValue: true, description: 'イメージをビルドしレジストリに登録')
    }
    ...
    stages {
        ...
        stage('GCP認証と設定') {
            steps {
                script {
                    withCredentials([file(credentialsId: "nakayama-gcp-cloud-run-deploy", variable: 'GCP_KEY_FILE')]) {
